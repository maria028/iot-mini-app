...
function SM4() {
  ...
  this.sm4_crypt_ecb = function (ctx, input) {
    ...
    for (var t = 0; t < length; t += 16) {
      ...
      this.sm4_one_round(ctx.sk, inn, out);
      bous = bous.concat(out);
    }
    if (ctx.isPadding && ctx.mode == this.SM4_DECRYPT) {
      bous = padding(bous, this.SM4_DECRYPT);
    }
...
const SM4Util = function (key) {
  ...
  this.encryptData_ECB = function (plainText) {
    try {
      ...
      const keyBytes = this.hexStrToBytes(this.secretKey);
  ...
  this.decryptData_ECB = function (cipherText) {
    try {
      ...
      ctx.isPadding = true;
      ctx.mode = sm4.SM4_ENCRYPT;
      let keyBytes = this.hexStrToBytes(this.secretKey);
  ...
  //16进制字符串转成16进制字节数组
  this.hexStrToBytes = function (hexStr) {
